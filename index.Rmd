...
tb %>% filter(country %in% c("Mexico", "Argentina")) %>% na.omit() %>% group_by(country, sex) %>% summarise(Total = sum(child)) %>% ggplot(mapping=aes(x=sex, y=Total))+geom_col(aes(fill=country))
```
...
tb %>% filter(country %in% c("Mexico", "Angola")) %>% na.omit() %>% group_by(country, sex) %>% summarise(Total = sum(child)) %>% ggplot(mapping=aes(x=sex, y=Total))+geom_col(aes(fill=country))
```
...
```{r, }
tb %>% filter(country == "Mexico") %>% select(country, sex, child, adult, elderly) %>% na.omit() %>% group_by(sex) %>% summarise(numero = sum(child, adult, elderly)) %>% ggplot(mapping= aes(x=sex, y=numero)) + geom_col()
```
...
```{r}
tb %>% filter(country == "Afghanistan", year == "2002") %>% select(country, sex, child, adult, elderly) %>% na.omit() %>% group_by(sex) %>% summarise(numero = sum(child, adult, elderly)) %>% ggplot(mapping= aes(x=sex, y=numero)) + geom_col()
```
...
```{r}
tb %>% filter(country == "United States of America", year >="1998", year <="2004") %>% select(country, sex, child, adult, elderly) %>% na.omit() %>% group_by(sex) %>% summarise(numero = sum(child, adult, elderly)) %>% ggplot(mapping= aes(x=sex, y=numero)) + geom_col()
```
